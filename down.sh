...
$ADD https://zlib.net/zlib-$ZLIB_VERSION.tar.gz > zlib.tar.gz

# validate
test "$(sha256sum nginx.tar.gz | awk '{print $1}')" == "$CHECKSUM" ] || echo "err: nginx.tar.gz"
test "$(sha256sum openssl.tar.gz | awk '{print $1}')" == "$OPENSSL_CHECKSUM" || echo "err: openssl.tar.gz"
test "$(sha256sum zlib.tar.gz | awk '{print $1}')" == "$ZLIB_CHECKSUM"  || echo "err: zlib.tar.gz"